...
- **协议是契约**：`shared/schemas/` 里的 `pointing_event` / `assistant_response` 是前后端唯一接口。物体目录的单一事实来源是 `shared/objects.json`（后端 `resolver.py` 读取它）。改字段要同时改两端，并在 `docs/protocol.md` 记版本。Quest 3 的相机字段是 **可选扩展**，不要改动 V0 已有字段。
